...
yearly_counts <- bind_rows(
  data %>%
    ...
    group_by(year) %>%
    summarise(count = n(), type = "Founded", .groups = "drop"),
  data %>%
    mutate(year = year(date_joined)) %>%
    group_by(year) %>%
    summarise(count = n(), type = "Joined", .groups = "drop")
)

ggplot(yearly_counts, aes(x = year, y = count, color = type)) +
  geom_line(linewidth = 1.1) +
  labs(
    title = "Unicorns Founded vs Joined Per Year",
    x = "Year",
